...
with open('homes.csv', 'w', newline='') as csvfile:
    ...
    for i in os.listdir("."):
        if i.endswith(".txt"):
            results = eval(open(i, "r").read())
            for result in results:
                try:
                    area = result["area"] if "area" in result else None
                    result = result["hdpData"]["homeInfo"]
                    if result["state"] != "NY":
                        continue
                    writer.writerow([
                        result["latitude"],
                        ...
                        i[:-4],
                        result["zipcode"],
                        median[int(result["zipcode"])],
                        result["zestimate"] if "zestimate" in result else None,
                        result["priceForHDP"],
                        result["bedrooms"],
                        result["bathrooms"],
                    ])
